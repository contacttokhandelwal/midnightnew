...
class Itactica_Intenso_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getGoogleFontsScript() {
		...
		if ($configFontFamily == 'google') {
				return '<link rel="stylesheet" href="https://fonts.googleapis.com/css?family='.$configGoogleFont.':'.$configFontStyles.'&amp;subset='.$configFontSubset.'" />';
		}
	}
    ...
    /**
     * return chosen CSS file if material design fields is enabled
     * @access public
     * @return string
     */
    public function getChosenCssFile() {
        if (Mage::getStoreConfig('intenso_design/page/form_fields_style', Mage::app()->getStore()) == 'mdformfields') {
            return 'css/chosen.css';
        }
        return null;
    }
     ...
     * get image height
     * @access public
     * @param array|Varien_Data_Collection $galleryImages
    ...
    public function getGalleryImagesHeight($galleryImages, $product, $width) {
        ...
        Mage::register('media_mobile_image_size', array($maxHeight,$resizeAsSquare,$baseImageSize));
    ...
    /**
     * return true if sticky menu is enabled
     * @access public
     * @param int
     * @return boolean
     */
    public function stickyMenu($storeId = NULL) {
        if ($storeId === NULL) {
            $storeId = Mage::app()->getStore();
        }
        return Mage::getStoreConfig('intenso_design/header/sticky_menu', $storeId);
    }

    /**
     * return sticky logo url
     * @access public
     * @param int
     * @return boolean
     */
    public function stickyMenuUrl($storeId = NULL) {
        if ($storeId === NULL) {
            $storeId = Mage::app()->getStore();
        }
        return Mage::getStoreConfig('intenso_design/header/logo_src_sticky', $storeId);
    }

    /**
     * checks if language switch for mobile is enabled
     * @access public
     * @param int
     * @return boolean
     */
    public function mobileLanguageSwitch() {
        if (Mage::getStoreConfig('intenso/footer/mobile_language_switch', Mage::app()->getStore())) {
            return true;
        }
        return false;
    }
    ...
    /**
     * checks if currency selector for mobile is enabled
     * @access public
     * @param int
     * @return boolean
     */
    public function mobileCurrencySelector() {
        if (Mage::getStoreConfig('intenso/footer/mobile_currency_selector', Mage::app()->getStore())) {
            return true;
        }
        return false;
    }
    ...
    /**
     * checks if store switch is enabled
     * @access public
     * @param int
     * @return boolean
     */
    public function mobileStoreSwitch() {
        if (Mage::getStoreConfig('intenso/header/store_switch', Mage::app()->getStore())) {
            return true;
        }
        return false;
    }
    ...
    /**
     * mobile footer switches
     * @access public
     * @param int
     * @return boolean
     */
    public function mobileFooterSwitches() {
        if ($this->mobileLanguageSwitch() || $this->mobileCurrencySelector() || $this->mobileStoreSwitch()) {
            return true;
        }
        return false;
    }
	...
	/**
	 * return true if the store switch is enabled
	 * @access public
	 * @param int
	 * @return boolean
	 */
	public function storeSwitcher($storeId = NULL) {
		if ($storeId === NULL) {
			$storeId = Mage::app()->getStore();
		}
		return Mage::getStoreConfig('intenso/header/store_switch', $storeId);
	}

	/**
	 * returns true if product gallery image slider is enabled
	 * @access public
	 * @return bool
	 */
	public function showGalleryImagesAsSlider() {
		return Mage::getStoreConfig('intenso/product_page/product_gallery_is_slider', Mage::app()->getStore());
	}
     ...
     * @return boolean
     */
    public function isInfiniteScroll($storeId = NULL) {
        if ($storeId === NULL) {
            $storeId = Mage::app()->getStore();
        }
        return Mage::getStoreConfig('intenso/catalog/infinite_scroll', $storeId);
    }
     ...
     * @return string
     */
    public function getRecentlyViewedProductImage($product)
    {
        $store = Mage::app()->getStore();
        $imageHeight = Mage::getStoreConfig('intenso/recently_viewed/image_height', $store);
        // set image
        if (Mage::getStoreConfig('intenso/recently_viewed/enable_adaptive_resize', $store)) {
            $adaptiveResizePosition = Mage::getStoreConfig('intenso/recently_viewed/adaptive_resize_position', $store);
            if ($adaptiveResizePosition == 2) {
                $cropPosition = 'top';
            } elseif ($adaptiveResizePosition == 3) {
                $cropPosition = 'bottom';
            } else {
                $cropPosition = 'center';
            }
            $image = Mage::helper('itactica_intenso/image')
                ...
                ->adaptiveResize(430, $imageHeight);
        } else {
            $image = Mage::helper('catalog/image')
                ->init($product, 'small_image')
                ->resize(430, $imageHeight);
        }
        return $image;
    }
     ...
     * @return int
     */
    public function getRecentlyViewedImageHeight() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/image_height', $store);
    }

    /**
     * @return string
     */
    public function getPictureStyleForRecentlyViewed() {
        $paddingTopRatio = 0.232549504950495;
        $paddingTopPercentage = $paddingTopRatio * $this->getRecentlyViewedImageHeight();
        return 'style="padding-top: ' . $paddingTopPercentage . '%;"';
    }
     ...
     * @return bool
     */
    public function getRecentlyViewedDisplayType() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/display_type', $store);
    }

    /**
     * @return bool
     */
    public function getRecentlyViewedHasPadding() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/has_padding', $store);
    }

    /**
     * @return bool
     */
    public function showRatingInRecentlyViewed() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/show_rating', $store);
    }

    /**
     * @return bool
     */
    public function showPriceTagInRecentlyViewed() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/show_price_tag', $store);
    }

    /**
     * @return bool
     */
    public function showAddToCartInRecentlyViewed() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/show_add_to_cart_button', $store);
    }

    /**
     * @return bool
     */
    public function showCompareInRecentlyViewed() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/show_compare_button', $store);
    }

    /**
     * @return bool
     */
    public function showWishlistInRecentlyViewed() {
        $store = Mage::app()->getStore();
        return Mage::getStoreConfig('intenso/recently_viewed/show_wishlist_button', $store);
    }
